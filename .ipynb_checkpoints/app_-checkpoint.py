...
# Python SQL and Object Relational Mapper
import sqlalchemy
...
def welcome():
    return (
        ...
        f"/api/v1.0/tobs<br/>"
        f"/api/v1.0/start (replace start with the set of YYYY-MM-DD)<br/>"
        f"/api/v1.0/start/end (replace start/end with the set of YYYY-MM-DD/YYYY-MM-DD)"
...
def tobs():
     ...
     .filter(Measurement.date>='2016-12-10').all()
...
if __name__ == '__main__':  #This line checks if the script is being run directly (as opposed to being imported as a module in another script). If it is, the code block that follows will be executed.
    app.run(debug=True)     #This line starts the Flask application. The debug=True argument enables debug mode, 
                            #which provides helpful error messages and automatically reloads the server when code changes are made.